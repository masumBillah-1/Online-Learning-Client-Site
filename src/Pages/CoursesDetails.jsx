...
useEffect(() => {
    // যদি user না থাকে বা course না থাকে, skip করো
    if (!user?.email || !singlecourse?._id) {
        setLoading(false);
        return;
    }
    ...
    const checkPurchase = async () => {
        try {
            ...
            // 🔍 Server থেকে check করছে
            const response = await fetch(
                `${API_URL}/purchases/check?email=${user.email}&courseid=${singlecourse._id}`
            );
            
            const data = await response.json();
            
            // ✅ Response অনুযায়ী state update
    ...
    const handleCoursePurchase = async (e) => {
        ...
        try {
            ...
            setIsPurchased(true); // 🔹 purchased state update
...
{loading ? (
  <button 
    className="w-full text-white py-3 rounded-lg font-semibold mb-3 opacity-60 cursor-not-allowed"
  ...
  <Link to="/my-courses" className="block">
    <button 
      className="w-full text-white py-3 rounded-lg font-semibold hover:opacity-90 transition-all mb-3 cursor-pointer"
  ...
  <button 
    className="w-full text-white py-3 rounded-lg font-semibold hover:opacity-90 hover:scale-105 transition-all mb-3 cursor-pointer shadow-lg"
    style={{background: 'linear-gradient(to right, #193485, #0a1c4a)'}}
    onClick={handelCourseModelOpen}
  >
    Enroll Now
  </button>
)}
